...
async function fetchUserProfile(apiKey) {
  try {
    ...
    userProfile = { plan: 'core', role: 'user', requestsRemaining: 500 };
    localStorage.setItem('userProfile', JSON.stringify(userProfile));
    updateRequestCounter();
  }
}
...
function updateDropdownOptions() {
  ...
  // No restrictions based on plan - all users have access to all options
...
async function handleFormSubmit(formId, resultId, endpoint) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (userProfile.requestsRemaining <= 0 && userProfile.role !== 'admin') {
      ...
      resultContent.innerHTML = '<p>Youve hit your monthly request limit. Upgrade your plan for more!</p><button onclick="window.location.href=\'/fitness/subscribe\'">Upgrade Now</button>';
