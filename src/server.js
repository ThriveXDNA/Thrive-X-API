...
// Updated subscription plans - removed Essential tier
...
const authenticateApiKey = async (req, res, next) => {
  ...
  try {
    ...
    if (data && !error) {
      req.user = { plan: data.plan || 'core', role: data.role || 'user' };
...
const limiter = rateLimit({
  ...
  message: (req) => ({
    ...
    tier_info: { current_tier: 'core', limit: 500, current_count: req.rateLimit?.current || 0, reset_after: Math.max(0, (req.rateLimit?.resetTime || 0) - Date.now()) / 1000 },
    upgrade_options: { next_tier: 'elite', benefits: ['2,000 requests/month'] }
  })
});
